...
{
    bool ok;
    QString url = QInputDialog::getText(this, tr("StreamOpen"),
                                                tr("StreamURL:"), QLineEdit::Normal,
                                               "rtmp://localhost/live/test", &ok);
    m_player->play(url);
}
void PlayerWindow::openMedia(QString url){
    m_player->stop();
    puts("Open media");
    m_player->play(url);
}
void PlayerWindow::openMedia(const QItemSelection& url){
    openMedia(url.indexes().first().data().toString());
}
